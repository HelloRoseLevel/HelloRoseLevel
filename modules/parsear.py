...
def categorizar_movimiento(descripcion):
    ...
    if any(word in desc for word in ["converted usd to eur", "converted usd", "usd to", "converted"]):
        return "Conversion"
    if any(word in desc for word in ["zelle", "venmo", "paypal","transfer", "external transfer",
                                     "online transfer", "auto routing", "wise", "truist", "chase", "mercuryach"]):
        return "Transfers"
    if any(word in desc for word in ["irs", "tax", "federal", "state", "impuesto"]):
        return "Taxes"
    if any(word in desc for word in ["restaurant", "grill", "starbucks", "mcdonald",
                                     "salad","coffee", "food", "bbq", "carrabba", "rent", "lease",
                                     "apartment", "uber", "amazon", "walmart", "target",
                                     "market", "mercado", "utility", "electric", "water",
                                     "internet", "phone", "spectrum", "comcast", "verizon", "at&t",
                                     "t-mobile", "sprint", "att", "tmobile", "verizon wireless",
                                     "sprint nextel", "chick-fil-a", "chipotle", "subway",
                                     "dunkin", "dairy queen", "burger king", "taco bell", "panera",
                                     "costco", "sam's club", "aldi", "whole foods", "kroger",
                                     "safeway", "publix", "heb", "food lion", "giant eagle",
...
def parsear_wise_usd(texto, nombre_archivo):
    ...
    if not df_movimientos.empty and "balance" in df_movimientos.columns:
        ultimo_balance = df_movimientos["balance"].iloc[-1]
        ultimo_monto = df_movimientos["monto"].iloc[-1]
        ultimo_tipo = df_movimientos["tipo"].iloc[-1]
        if ultimo_tipo == "egreso":
            saldo_inicial = ultimo_balance + ultimo_monto
        else:  
            saldo_inicial = ultimo_balance - ultimo_monto
    else:
