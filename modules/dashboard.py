...
# Filtrar solo egresos de suscripciones
        ...
        with col_pie:
            ...
            mask_suscripciones = df['categoría'].str.lower() == 'subscriptions/services'.lower()
            egresos_suscripciones = (
                df[(df['tipo'].str.lower() == 'egreso') & mask_suscripciones]
                .groupby('descripción')['monto']
                .sum()
                .sort_values(ascending=False)
            )
            egresos_suscripciones = egresos_suscripciones[egresos_suscripciones > 0]

            top_n = 10
            if len(egresos_suscripciones) > top_n:
                top = egresos_suscripciones.head(top_n)
                otros = egresos_suscripciones.iloc[top_n:].sum()
                egresos_suscripciones = pd.concat([top, pd.Series({'Otros': otros})])
            if not egresos_suscripciones.empty:
                fig_pie, ax_pie = plt.subplots(figsize=(2.2, 2.2))
                ax_pie.pie(
                    egresos_suscripciones,
                    labels=egresos_suscripciones.index,
                    autopct='%1.1f%%',
                    startangle=140,
                    counterclock=False,
                    textprops={'fontsize': 7}
                )
                ax_pie.set_title("Egresos por suscripción", fontsize=9)
                st.pyplot(fig_pie)
            else:
                st.info("No hay egresos para mostrar por suscripción.")
